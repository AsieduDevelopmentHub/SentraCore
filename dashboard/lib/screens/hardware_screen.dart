...
/// "Hardware" destination — CrystalDiskInfo-style health summary for the
/// machine. Pulls from the engine's `/api/v1/hardware/health` endpoint and
/// refreshes on a slow interval (the engine itself caches probe results for
/// 30 s, so the UI doesn't need to be aggressive).
...
class _HardwareScreenState extends State<HardwareScreen> {
  ...
  void initState() {
    super.initState();
    unawaited(_refresh(refresh: true));
  ...
  Future<void> _refresh({bool refresh = false}) async {
    setState(() {
      _loading = true;
      _error = null;
    });
    ...
    setState(() {
      _loading = false;
      if (data == null) {
        _error = 'No response from the engine.';
      } else if (data['ok'] == false) {
        _error = (data['error'] as String?) ?? 'Hardware probe failed.';
      } else {
        _report = data;
      }
    });
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                'Health of CPU, RAM, and storage devices.',
                style: TextStyle(
