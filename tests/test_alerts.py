"""Tests for AlertManager."""

from engine.alerts.alert_manager import Alert, AlertManager
from engine.process.process_tracker import ProcessImpact
from engine.stress.stress_engine import StressResult
from engine.user_preferences import UserPreferences


def _make_stress(score: float, level: str = "high") -> StressResult:
    """Pressures mirror score so defaults (85/85/80) breach when score >= 90."""
    p = min(100.0, max(0.0, score))
    return StressResult(
        score=score,
        level=level,
        cpu_pressure=p,
        memory_pressure=p,
        disk_pressure=p,
        weights={"cpu": 0.4, "memory": 0.35, "disk": 0.25},
    )


def _make_process(name: str = "test.exe") -> ProcessImpact:
    return ProcessImpact(
        pid=1,
        name=name,
        avg_cpu_percent=30.0,
        avg_memory_percent=10.0,
        peak_cpu_percent=50.0,
        peak_memory_percent=15.0,
        current_cpu_percent=30.0,
        current_memory_percent=10.0,
        sample_count=5,
        impact_score=22.0,
    )


class TestAlertManager:
    def test_no_alert_on_low_stress(self):
        mgr = AlertManager(consecutive_count=3)
        result = mgr.evaluate(_make_stress(30.0, "low"), [_make_process()])
        assert result is None

    def test_no_alert_before_consecutive_threshold(self):
        mgr = AlertManager(consecutive_count=3, cooldown_sec=0.0)

        mgr.evaluate(_make_stress(90.0), [_make_process()])
        result = mgr.evaluate(_make_stress(90.0), [_make_process()])
        assert result is None

    def test_alert_fires_at_consecutive_threshold(self):
        mgr = AlertManager(consecutive_count=3, cooldown_sec=0.0)

        mgr.evaluate(_make_stress(90.0), [_make_process()])
        mgr.evaluate(_make_stress(90.0), [_make_process()])
        result = mgr.evaluate(_make_stress(90.0), [_make_process()])

        assert isinstance(result, Alert)
        assert result.stress_score == 90.0
        assert "sustained" in result.message.lower()

    def test_cooldown_prevents_spam(self):
        mgr = AlertManager(consecutive_count=2, cooldown_sec=60.0)

        mgr.evaluate(_make_stress(90.0), [_make_process()])
        alert1 = mgr.evaluate(_make_stress(90.0), [_make_process()])
        assert alert1 is not None

        alert2 = mgr.evaluate(_make_stress(90.0), [_make_process()])
        assert alert2 is None
        assert mgr.is_in_cooldown is True

    def test_consecutive_resets_on_drop(self):
        mgr = AlertManager(consecutive_count=3, cooldown_sec=0.0)

        mgr.evaluate(_make_stress(90.0), [_make_process()])
        mgr.evaluate(_make_stress(90.0), [_make_process()])
        mgr.evaluate(_make_stress(50.0, "moderate"), [_make_process()])
        result = mgr.evaluate(_make_stress(90.0), [_make_process()])

        assert result is None

    def test_callback_called(self):
        alerts_received = []
        mgr = AlertManager(consecutive_count=1, cooldown_sec=0.0)
        mgr.register_callback(lambda a: alerts_received.append(a))

        mgr.evaluate(_make_stress(90.0), [_make_process()])
        assert len(alerts_received) == 1

    def test_total_alerts_counter(self):
        mgr = AlertManager(consecutive_count=1, cooldown_sec=0.0)
        mgr.evaluate(_make_stress(90.0), [_make_process()])
        mgr.evaluate(_make_stress(90.0), [_make_process()])

        assert mgr.total_alerts == 2

    def test_alert_to_dict(self):
        mgr = AlertManager(consecutive_count=1, cooldown_sec=0.0)
        alert = mgr.evaluate(_make_stress(90.0), [_make_process("chrome.exe")])

        d = alert.to_dict()
        assert "timestamp" in d
        assert "stress_score" in d
        assert "top_contributors" in d
        assert "message" in d
        assert "root_cause" in d
        assert d["root_cause"] is not None
        assert "primary_bottleneck" in d["root_cause"]

    def test_reset(self):
        mgr = AlertManager(consecutive_count=1, cooldown_sec=60.0)
        mgr.evaluate(_make_stress(90.0), [_make_process()])
        mgr.reset()

        assert mgr.total_alerts == 0
        assert mgr.is_in_cooldown is False
        assert mgr.consecutive_high_count == 0

    def test_cooldown_remaining_after_fire(self):
        mgr = AlertManager(consecutive_count=1, cooldown_sec=120.0)
        mgr.evaluate(_make_stress(90.0), [_make_process()])
        assert mgr.cooldown_total_sec == 120.0
        assert mgr.cooldown_remaining_sec > 0
        assert mgr.is_in_cooldown is True

    def test_cooldown_remaining_zero_before_first_alert(self):
        mgr = AlertManager(consecutive_count=3, cooldown_sec=60.0)
        assert mgr.cooldown_remaining_sec == 0.0
        assert mgr.is_in_cooldown is False

    def test_custom_thresholds_delay_alert(self):
        prefs = UserPreferences(
            alert_cpu_percent=95.0,
            alert_memory_percent=95.0,
            alert_disk_pressure=95.0,
        )
        mgr = AlertManager(consecutive_count=1, cooldown_sec=0.0)
        assert mgr.evaluate(_make_stress(90.0), [_make_process()], prefs=prefs) is None
