...
def _make_snapshot(procs=(), ts=None) -> SystemSnapshot:
    return SystemSnapshot(
        ...
        cpu_percent=25.0, cpu_per_core=(25.0,), cpu_count_logical=1,
        memory_total=16_000_000_000, memory_used=8_000_000_000,
        memory_available=8_000_000_000, memory_percent=50.0,
        swap_total=4_000_000_000, swap_used=0, swap_percent=0.0,
...
def _make_normalized(cpu_spike=False, mem_spike=False, disk_spike=False, ts=None) -> NormalizedSnapshot:
    return NormalizedSnapshot(
        timestamp=ts or time.time(),
        cpu_percent_smoothed=50.0, memory_percent_smoothed=50.0,
        cpu_percent_raw=50.0, memory_percent_raw=50.0,
        memory_used=8_000_000_000, memory_available=8_000_000_000,
        memory_total=16_000_000_000, swap_percent=0.0,
        disk_read_bytes_per_sec=0.0, disk_write_bytes_per_sec=0.0,
        disk_read_ops_per_sec=0.0, disk_write_ops_per_sec=0.0,
        disk_total_bytes_per_sec=0.0, disk_total_ops_per_sec=0.0,
        cpu_is_spiking=cpu_spike, memory_is_spiking=mem_spike,
...
class TestEventLogger:
    ...
    def test_cpu_spike_no_duplicate(self):
        ...
        resolve_events = [e for e in events3 if e.event_type == "cpu_spike" and e.severity == "info"]
        assert len(resolve_events) == 1

    def test_process_start_detection(self):
        logger = EventLogger()
        proc1 = ProcessInfo(pid=100, name="a.exe", cpu_percent=10.0,
                           memory_percent=5.0, memory_rss=100, status="running", create_time=1.0)

        # First call establishes known pids
        logger.analyze(_make_snapshot(procs=[proc1]), _make_normalized())

        # Second call with new process
        proc2 = ProcessInfo(pid=200, name="b.exe", cpu_percent=5.0,
                           memory_percent=3.0, memory_rss=50, status="running", create_time=2.0)
        events = logger.analyze(_make_snapshot(procs=[proc1, proc2]), _make_normalized())
